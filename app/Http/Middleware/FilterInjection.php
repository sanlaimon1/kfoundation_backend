        ...
        if(!empty($input))
        {
            foreach( $input as $key=>$value ) {
                ...
                else
                {
                    ...
                    if(preg_match("/select|update|insert|delete|and|or|where|union|join|create|drop|like|eval|open|sysopen|system|passwd|alter|char|\"|\'|\?/", $value) )
                    {
                        $arr = ['code'=>-30, 'message'=> $key . '里包含非法参数'];
