    ...
    <div class="container-fluid">
        ...
        <nav aria-label="breadcrumb" style="margin-top: 1rem;">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('withdrawal.index') }}">余额提现列表</a></li>
                <li class="breadcrumb-item active" aria-current="page">查看详情</li>
            </ol>
        </nav>

        <h3 class="text-center text-primary">余额提现详情</h3>
        ...
        <ul class="list-group list-group-flush" style="margin-top:1rem;">
            ...
            <li class="list-group-item">
                <span style="float:left;">
                    <strong>状态:</strong> {{ $status[ $one->status ] }}
                </span>
                @if($one->status==0)
                <form action="{{ route('withdrawal.update', ['withdrawal'=>$one->id]) }}" 
                         method="post"
                ...
                <a style="margin-left: 8rem; float:left;" href="{{ route('withdrawal.edit', ['withdrawal'=>$one->id]) }}" class="btn btn-danger">拒绝</a>
                @endif
            </li>
            @if($one->status!=0)
            <li class="list-group-item">
